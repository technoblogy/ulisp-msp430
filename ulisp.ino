/* uLisp MSP430 Version 1.8  - www.ulisp.com
...
#define BUFFERSIZE 18
  ...
  #define WORKSPACESIZE 3328              /* Cells (4*bytes) */
  #define IMAGEDATASIZE 1664              /* Cells */
...
int saveimage (object *arg) {
  ...
  // Save to Flash - should always fit
...
inline uint8_t I2Cread(uint8_t last) {
  return Wire.read();
}
...
#if defined(__MSP430F5529__)
  if (!(pin==2 || pin==6 || (pin>=23 && pin<=28))) error(F("'analogread' invalid pin"));
#elif defined(__MSP430FR5969__)
  if (!(pin==5 || (pin>=11 && pin<=13) || pin==18 || pin==19 || pin==23 || pin==24)) error(F("'analogread' invalid pin"));
#endif
}

void checkanalogwrite (int pin) {
#if defined(__MSP430F5529__)
  if (!(pin==12 || pin==19 || (pin>=35 && pin<=40))) error(F("'analogwrite' invalid pin"));
#elif defined(__MSP430FR5969__)
  if (!(pin==12 || pin==19 || (pin>=6 && pin<=15) || pin==18 || pin==19 || 
  pin==21 || pin==22 || pin==26)) error(F("'analogwrite' invalid pin"));
#endif
}
...
void playnote (int pin, int note, int octave) {
  ...
  if (prescaler<0 || prescaler>8) error(F("'note' octave out of range"));
